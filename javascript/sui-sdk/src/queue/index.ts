...
function normalizeTypeName(typeName: string): string {
  return typeName.replace(
    /\b([0-9a-fA-F]{64})(?=::)/g,
    (_, address: string) => `0x${address.replace(/^0+/, '') || '0'}`
  );
}

function parseNonzeroHex(value: string, byteLength: number, field: string) {
  ...
  if (
    ...
    throw new Error(`${field} must be a ${byteLength}-byte hex value`);
  }

  if (/^0+$/.test(normalized)) {
    throw new Error(`${field} must not be all zero`);
  }
...
export class Queue {
  ...
  public async overrideOracleTx(
    ...
    const secp256k1Key = parseNonzeroHex(
      options.secp256k1Key,
      64,
      'secp256k1Key'
    );
    const mrEnclave = parseNonzeroHex(options.mrEnclave, 32, 'mrEnclave');
    const { switchboardAddress } = await this.client.fetchState(options);
    tx.moveCall({
      ...
      arguments: [
        ...
        tx.pure.vector('u8', Array.from(secp256k1Key)),
        tx.pure.vector('u8', Array.from(mrEnclave)),
  ...
  public async loadData(): Promise<QueueData> {
    ...
    do {
      const existingOraclesDynamicFields =
        await this.client.client.getDynamicFields({
          parentId,
          cursor,
        });
      existingOraclesObjects.push(...existingOraclesDynamicFields.data);
      cursor = existingOraclesDynamicFields.hasNextPage
        ? existingOraclesDynamicFields.nextCursor
        : null;
    } while (cursor);
          ...
          try {
            ...
            if (typeof ft === 'object' && ft !== null && 'fields' in ft) {
              const fields = (ft as { fields: Record<string, MoveValue> })
                .fields;
              return fields.name
                ? normalizeTypeName(ObjectParsingHelper.asString(fields.name))
                : '';
            }
            return '';
          } catch {
