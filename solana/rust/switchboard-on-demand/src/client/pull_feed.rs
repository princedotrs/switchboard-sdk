use super::accounts::{OracleAccountData, PullFeedAccountData, QueueAccountData, State};
use super::crossbar::CrossbarClient;
use super::gateway::{
    encode_jobs, BatchFeedRequest, ConsensusOracleResponse, FetchSignaturesConsensusResponse,
...
use crate::{Pubkey, NATIVE_MINT, SPL_TOKEN_PROGRAM_ID, SYSTEM_PROGRAM_ID};
use anyhow::anyhow;
use anyhow::Context;
...
fn decode_pull_feed_account_data(account: &[u8]) -> Result<PullFeedAccountData, AnyhowError> {
    ...
    // Anchor account data starts after an eight-byte discriminator, which does
    // not guarantee the alignment required by the i128-containing client type.
    ...
    fn get_solana_submit_signatures_ix(
        ...
        for resp in &responses {
            remaining_accounts.push(
                crate::solana_compat::solana_sdk::instruction::AccountMeta::new_readonly(
                    resp.oracle.to_bytes().into(),
                    false,
                ),
            );
        }
        for resp in responses {
            ...
            remaining_accounts.push(
                crate::solana_compat::solana_sdk::instruction::AccountMeta::new(
                    stats_key.to_bytes().into(),
                    false,
                ),
            );
        }
        let mut submit_ix = Instruction {
            ...
            .into_iter()
            .map(
                |meta| crate::solana_compat::solana_sdk::instruction::AccountMeta {
