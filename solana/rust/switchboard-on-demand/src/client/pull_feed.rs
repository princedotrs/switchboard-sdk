...
use crate::{NATIVE_MINT, Pubkey, SPL_TOKEN_PROGRAM_ID, SYSTEM_PROGRAM_ID};
...
impl PullFeed {
    ...
    fn get_solana_submit_signatures_ix(
        ...
        for resp in &responses {
            remaining_accounts.push(crate::solana_compat::solana_sdk::instruction::AccountMeta::new_readonly(resp.oracle.to_bytes().into(), false));
        }
        for resp in responses {
            let stats_key = OracleAccountData::stats_key(&resp.oracle);
            remaining_accounts.push(crate::solana_compat::solana_sdk::instruction::AccountMeta::new(stats_key.to_bytes().into(), false));
        }
        let mut submit_ix = Instruction {
            ...
            .into_iter()
            .map(|meta| crate::solana_compat::solana_sdk::instruction::AccountMeta {
    ...
    pub async fn fetch_update_consensus_ix(
        ...
        for feed in &feed_pubkeys {
            remaining_accounts.push(crate::solana_compat::solana_sdk::instruction::AccountMeta::new(feed.to_bytes().into(), false));
        }
        for oracle in oracle_keys.iter() {
            remaining_accounts.push(crate::solana_compat::solana_sdk::instruction::AccountMeta::new_readonly(oracle.to_bytes().into(), false));
