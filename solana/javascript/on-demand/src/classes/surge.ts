...
interface RawGatewayResponse {
  ...
  heartbeat_at_ts_ms?: number; // Timestamp when heartbeat was triggered (only present when triggered_on_price_change is false)
  message?: string;
}
...
export class SurgeUpdate {
  ...
  getLatencyMetrics(): {
    ...
    const broadcastTimeMs = this.rawResponse.broadcast_ts_ms ?? 0;
    const isHeartbeat = this.rawResponse.triggered_on_price_change === false;
    const heartbeatAtMs =
      this.rawResponse.heartbeat_at_ts_ms ?? broadcastTimeMs;
    ...
    const bundleEmitLatencyMs =
      perFeedMetrics.length > 0
